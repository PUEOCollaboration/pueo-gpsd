 ...
 * For Boreas D90
 * Documentation here: https://docs.advancednavigation.com/boreas-d/ANPP/Advanced%20Navigation%20Packet.htm
 *
 * Adapted from UBX driver, 'driver_ubx.c', since this is also a binary protocol IMU
 */
...
/* ------------------------- Custom gpsd functions for ANPP ------------------------ */

/* send a anpp message.
 * calculate checksums, etc.
 */
bool anpp_write(struct gps_device_t * session,
...
static gps_mask_t anpp_parse(struct gps_device_t * session, unsigned char *buf,
                            size_t len)
{
  return 0;
}
...
static ssize_t anpp_control_send(struct gps_device_t *session, char *msg,
                                size_t data_len)
{
  return 0;
}

static void anpp_init_query(struct gps_device_t *session)
{
}
...
static gps_mask_t anpp_cfg_prt(struct gps_device_t *session, speed_t speed,
                              const char parity,
...
static void anpp_mode(struct gps_device_t *session, int mode)
{
}

static bool anpp_speed(struct gps_device_t *session,
...
static bool anpp_rate(struct gps_device_t *session, double cycletime)
{
  return true;
}
